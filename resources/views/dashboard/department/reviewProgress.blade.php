...
<div class="container-fluid mt-5">
    <div class="card bg-white border-0 rounded-4 shadow">
        <div class="card-body user-card">
            ...
            <form method="GET" action="{{ route('department.reviews') }}" class="row mb-4 g-3 align-items-end">
                <div class="col-md-4">
                    <label for="status" class="form-label">{{ __('Filter by Status') }}</label>
                    <select class="form-select" name="status" id="status">
                        <option value="">{{ __('All Statuses') }}</option>
                        @foreach (['approved','revision_required','pending','rejected','resubmitted'] as $stat)
            ...
            @if ($reviews->isEmpty()) 
                <p>{{ __('No papers reviewed under this department') }}</p>
            @else
