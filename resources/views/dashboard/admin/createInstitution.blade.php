...
<div class="row g-4">
    <div class="col-lg-4">
        <div class="card bg-white border-0 rounded-4 shadow">
            <div class="card-body user-card">
                <h5 class="card-title mb-3 fw-bold">{{ __('Create Institution') }}</h5>
                <form id="userForm" action="{{ route('institution.store') }}" method="POST">
                    @csrf
                    <input type="hidden" name="role" id="role" value="institution">
                    <input type="hidden" id="editUserId" name="editUserId">
                    <input type="hidden" name="_method" id="formMethod" value="POST">
                    <div class="col-12 mb-2">
                        <label for="name">{{ __('Name of the Institution') }}</label>
                    ...
                    {{-- <div class="col-12 mb-3">
                        <label for="gst">{{ __('GST')}}</label>
                        <input type="text" class="form-control w-100" name="gst" id="gst">
                        <div id="error-gst" class="text-danger"></div>
                    </div> --}}
                     <div class="col-12 mb-2">
    ...
    <div class="col-lg-8">
        <div class="card bg-white border-0 rounded-4 shadow">
            <div class="card-body user-card">
                <h5 class="card-title mb-3 fw-bold">{{ __('View Institution') }}</h5>
                <div class="table-responsive">
                    <table class="table table-bordered border-dark-subtle table-hover fs-6" id="instituteTable">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const instituteTable = document.querySelector("#instituteTable tbody");
    ...
    userForm.addEventListener("submit", async function (e) {
        ...
        const errorMessages = {
            name: "{{ __('Please enter the name') }}",
            mobile_number: "{{ __('Please enter the mobile number') }}",
            email: "{!! __('Please enter the email') !!}",
            password: "{{ __('Password must be at least 6 characters long') }}"
        };
        ...
        fields.forEach((id) => {
            ...
            if (!input.value.trim()) {
                error.textContent = errorMessages[id] || `Please enter the ${id}`;
                hasError = true;
        ...
        if (isEditing) {
            url = "{{ route('institution.update', ':id') }}".replace(':id', editUserIdInput.value);
        ...
        try {
            ...
            if (response.ok) {
                showPopup(result.message || "Institution saved successfully");
                userForm.reset();
                ...
                showPopup(`Failed to save institution: ${result.message || response.statusText}`);
            }
        } catch (error) {
    ...
    instituteTable.addEventListener("click", async function (e) {
        ...
        if (target.classList.contains("del-btn")) {
            showPopup("This will delete the institution permanently!", true, async () => {
                const url = "{{ route('institution.destroy', ':id') }}".replace(':id', userId);
                try {
                    const response = await fetch(url, {
                        method: "DELETE",
                        headers: {
                    ...
                    const result = await response.json();
                    if (response.ok) {
                        showPopup("Institution deleted successfully");
                        fetchUsers();
                    } else {
                        showPopup(`Failed to delete institution: ${result.message || response.statusText}`);
                    }
                } catch (error) {
    ...
    async function fetchUsers() {
        try {
            const response = await fetch("{{ route('institution.list') }}", {
                method: "GET",
                headers: {
                    "Accept": "application/json",
                    "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content,
            ...
            showPopup(`Failed to load institutions: ${error.message}`);
        }
    }

    function renderUsers() {
        instituteTable.innerHTML = "";
        users.forEach((user, index) => {
            const row = instituteTable.insertRow();
